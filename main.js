...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "Super"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
try {
const listaDirectorios = readdirSync('./Super/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./Super/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./Super/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./Super/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./Session/', './Super/']
